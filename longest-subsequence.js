...
// A string can be a subseqence of another string if all of its characters appear and are in the same order.For example, 'hello' is a subseqence of 'help love everyone' because 'h', 'e', 'l', 'l' and 'o' appear in the second string in the same order as 'hello'.
...
function isSubsequence(word, object) {
    let index = 0;
    for (const letter of word) {
        if (object[letter]) {
            index = findNextIndex(object[letter], index);
            if (index === false) {
                return false;
            }
        } else {
...
//So now we have all the helper function. Let's put all puzle pices together and make the final function of
//our problem
